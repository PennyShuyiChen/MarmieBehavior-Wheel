classdef PR_IBLContrastGabor < handle
  ...
  methods (Access = public)
    ...
    function generate_trialsList(o,S,P)  % the randmized trial list 
        ...
        cs = [0.9, 0.95, 0.975, 1];%[0.015, 0.03, 0.06, 0.125, 0.25];%[0,logspace(log10(0.01),log10(0.25),9)];
        %cs = [0.125, 0.25] easy contrasts
    ...
    function drop = state_and_screen_update(o,currentTime,x,y,inputs,~)
        ...
        if o.state == 3 && currentTime <= o.stimStart + o.P.noresponseDur
            ...
            % Eye must remain in the fixation window during
            % NOTE: Could change from fixWinRadius to another less strict
            % parameter if this proves too hard
            if norm([x y]) > o.P.fixWinRadius
                o.state = 8; % Move to iti -- inter-trial interval
                o.error = 5; % Error 5 is failure to hold fixation during
                o.itiStart = GetSecs;
            end

            if abs(o.hProbe.position(1)  - o.S.centerPix(1)) < abs(o.P.bound) 
        ...
        switch o.state
            ...
            case 8
                if (o.error == 2 || o.error == 5) % broke fixation
